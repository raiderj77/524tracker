# 524 Tracker — CLAUDE.md

> Source of truth for Claude Code on this project. Last updated: 2026-03-11
...
- Affiliate partner: CJ Affiliate. Affiliate links must include `rel="nofollow sponsored"`.
...
- Describe editorial review process (especially for YMYL-Adjacent content)
- Attribution: "Built by an experienced web professional" — NEVER use personal name
- Content authority from professional web development and financial tools experience.
...
- Third-party recipients (including Google for AdSense, CJ Affiliate)
...
- All card tracking data is stored in localStorage only — never transmitted to any server
- Privacy policy must clearly state that card application data never leaves the user's device
- No analytics or tracking on localStorage contents
...
10. Never guarantee card approval or bonus eligibility
11. Never provide financial advice — informational only
12. Never transmit localStorage card tracking data to any server
13. Never omit affiliate disclosure on card recommendation pages
14. Never omit last-verified dates on bank rules pages

## YMYL-Adjacent Language Rules
- Never use: guaranteed approval, guaranteed bonus, financial advice, invest
- Always use: may be eligible, check with your bank, for informational purposes only, rules may change
...
## Credit Card Rules Content Rules

### Bank Rules (must include last-verified dates)
- Chase 5/24: Applications denied if 5+ new cards from any bank in last 24 months
- Amex once-per-lifetime: Welcome bonus limited to once per card product per lifetime
- Citi 8/65: Max 1 app per 8 days, max 2 per 65 days
- Bank of America 2/3/4: Max 2 cards per 30 days, max 3 per 12 months, max 4 per 24 months
- Capital One: Generally limits to 2 consumer cards
- Rules change frequently — always display last-verified date on every rules page

### Required Disclaimer (on EVERY tool and recommendation page)
...
### Data Handling
- All card application tracking data stored in localStorage only
- No server-side storage of user financial data
- No cookies for tracking card application data
